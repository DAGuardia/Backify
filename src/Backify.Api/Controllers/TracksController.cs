...
[ApiController]
[Route("tracks")]
public class TracksController(TracksOrchestrator orchestrator, SpotifyService spotifyService) : ControllerBase
{
    ...
    public async Task ApplyStream([FromQuery] string ids)
    {
        ...
        var batches = trackIds.Chunk(40).ToList();
        int done = 0;
        ...
        try
        {
            ...
            {
                await spotifyService.LikeTracksAsync(sp!, batch);
                done += batch.Length;
